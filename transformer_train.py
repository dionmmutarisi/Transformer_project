"""
Assignment 3B — Questions 10–14 + Wikipedia extension
DAS5-ready: argparse CLI, W&B logging, Wikipedia + toy data, Optuna tuning.

Usage:
    python transformer_train.py --mode toy                        # quick test
    python transformer_train.py --mode wp --emb 512 --heads 8    # Q14 full run
    python transformer_train.py --mode tune --n-trials 30         # HP sweep
"""
...
class TransformerBlock(nn.Module):
    def __init__(self, emb, heads, mask, ff_mult=4, dropout=0.1):
        super().__init__()
        self.attn  = MultiHeadSelfAttention(emb, heads, mask)
...
class AutoregressiveTransformer(nn.Module):
    def __init__(self, vocab, emb=300, heads=6, depth=6, context=256, dropout=0.1):
        ...
        self.pos_emb = nn.Embedding(context, emb)
...
if __name__ == "__main__":
    ...
    if args.mode == "tune":
        ...
        run_name = args.wandb_name or f"{args.mode}-run"
        with wandb.init(entity=ENTITY, project=PROJECT, name=run_name,
